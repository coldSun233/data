    ...
    for cf = 1:targetTensorDim
        if(~isempty(Z.cpdim{cf}))
            ...
            for n = 1:num-1
                idx1 = cpTensor(n);
                idx2 = cpTensor(n+1);
                f = f + (norm(A{idx1}{cf} - A{idx2}{cf},'fro')^2) / (weightsOfF(cf));
            end
            idx = cpTensor(num);
            f = f + (norm(A{1}{cf} - A{idx}{cf},'fro')^2) / (weightsOfF(cf));
        end
    end
    ...
    for cf = 1:targetTensorDim
        if(~isempty(Z.cpdim{cf}))
            ...
            for n = 1:num
                ...
                Gp{n}{cf} = Gp{n}{cf} + 2*(2*A{idx}{cf} - A{idx1}{cf} - A{idx2}{cf}) / weightsOfF(cf);
            end
        end
    end    
    G = Gp;
